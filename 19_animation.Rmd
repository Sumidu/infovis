...
## Animationen können den Leser fesseln
...
p <- gdp_ranked %>%
  ...
  ggplot(
    aes(gdp, rank, group = country,color = highlight, fill = highlight)
  ) +
  geom_col(alpha = 0.8) +
  geom_text( aes(x = -200, label = country), hjust = 1, size = 12/.pt ) +
  geom_text( aes(x = gdp + 200, label = label), hjust = 0, size = 12/.pt ) +
  scale_color_manual(
  ...
  labs(
    title = "Chinas wachsendes BIP",
    subtitle = "Jahr: {closest_state}"
  ) +
...
p <- gdp_ranked %>%
  ...
  scale_y_log10(name = "BIP (Mrd USD)", labels = scales::comma) +
  scale_color_manual(
...
## Animationen in R werden mit **gganimate** erstellt
...
Das **gganimate** Paket ergänzt **ggplot2** um mächtige Werkzeuge für Animationen
...
## Wie sollte man über Animationen nachdenken?

---

## Animationen sollte man sich als Facetierung über die Zeit vorstellen
...
## Wir wissen wie man einen facetierten Plot macht
...
## Eine Animation zu erstellen ist in etwa genauso einfach
...
## Ländername und Plottitel hinzufügen
...
## Eine Zeitreihe erstellen wir mit `transition_reveal()`
...
## Das funktioniert auch mit **ggrepel** für das Labeln
